...
static std::vector<std::tuple<std::string, int, int, int, std::string, int>> previous_client_list;
...
void update_process_statuses() {
    int client_id = 0;
    std::string message = "Brak"; // Domyślny komunikat
    ...
    for (auto& client : global_client_list) {
        client_id = std::get<3>(client);
        message = "Brak"; // Domyślny komunikat
        timestamp = 0;           // Domyślny timestamp

        {
            std::lock_guard<std::mutex> msg_lock(last_messages_mutex);
            auto it = std::find_if(
                last_messages.begin(),
                last_messages.end(),
                [client_id](const std::pair<int, std::pair<std::string, int>>& entry) {
                    return entry.first == client_id;
                });

            if (it != last_messages.end()) {
                message = it->second.first;   // Pobieramy wiadomość
                timestamp = it->second.second; // Pobieramy timestamp
                if (debug)
                    std::cout << "[DEBUG] Process ID " << client_id << ": Last message updated to " << message << ", Timestamp: " << timestamp << std::endl;
            } else {
                if (debug)
                    std::cout << "[DEBUG] Process ID " << client_id << ": No last message found." << std::endl;
            }
        }
...
void update_other_processes_view() {
    ...
    {
        std::lock_guard<std::mutex> lock(global_client_list_mutex);
        if (global_client_list == previous_client_list) {
            if (debug)
                std::cout << "[DEBUG] No changes in client list. Skipping update." << std::endl;
            return;
        }

        // Aktualizujemy previous_client_list
        previous_client_list = global_client_list;
    }
    ...
    {
        std::lock_guard<std::mutex> lock(global_client_list_mutex); // Dodajemy blokadę
...
void update_logs()
{
    auto update_function = [] (gpointer data) -> gboolean {
        ...
        {
            std::lock_guard<std::mutex> lock(logs_mutex); // Dodajemy blokadę
